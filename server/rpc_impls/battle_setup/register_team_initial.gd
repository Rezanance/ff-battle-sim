...
static func _on_battle_prep_timeout(battle_id: int) -> void:
	...
	for player_id: int in ServerVariables.battle_teams[battle_id]:
...
static func notify_battle_prep_started(battle_id: int) -> void:
	...
	var player1_id: int = ServerVariables.battle_teams[battle_id].keys()[0]
	var player2_id: int = ServerVariables.battle_teams[battle_id].keys()[1]
