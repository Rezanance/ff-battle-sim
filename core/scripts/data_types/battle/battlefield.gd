...
func get_opponent_id(player_id: int):
	return formations.keys().filter(func(id): return id != player_id)[0]

func apply_support_effects(player_id: int):
	var opponent_id = get_opponent_id(player_id)
	var player_az_support_effects = formations[player_id].az_support_effects
	var opponent_az_support_effects = formations[opponent_id].az_support_effects
	var sz_vivosaurs = formations[player_id].get_sz_vivosaurs()
