...
const WhoGoesFirst = preload("res://server/rpc_impls/turns/who_goes_first.gd")

@rpc("any_peer", "call_remote", "reliable")
func who_goes_first_server(battle_id: int) -> void:
	...
	var player_1_total_lp: int = WhoGoesFirst.calculate_total_lp(battle_id, player_1)
	var player_2_total_lp: int = WhoGoesFirst.calculate_total_lp(battle_id, player_2)
	
	var battlefield: BattleField = ServerVariables.battlefields[battle_id]
...
@rpc("any_peer", "call_remote", "reliable")
func end_turn_server(battle_id: int) -> void:
	assert(multiplayer.is_server())
	var battlefield: BattleField = ServerVariables.battlefields[battle_id]
