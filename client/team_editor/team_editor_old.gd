...
var MedalImage = preload("res://client/team_editor/fossilary/medal_image.tscn")
...
func _add_fossilary_medals():
	for vivosaur_id in Constants.fossilary:
...
func _position_team_medals():
	for slot in range(len(team_slot_medal_btns)):
		var medal_btn = team_slot_medal_btns[slot]
		if medal_btn != null:
...
func _create_medal_fossilary(_texture, vivosaur_id: int, medal_btn: BaseButton):
	var medal_fossilary = MedalImage.instantiate()
...
func _show_context_menu(event: InputEventMouseButton, medal_btn: BaseButton):
	context_menu.clear()
	if not medal_btn in team_slot_medal_btns:
...
func _assign_slot(slot: int):
	...
	team.slots[slot] = Constants.fossilary[currently_selected_vivosaur_id]
	
	team_slot_medal_btns[slot] = duplicate_medal_btn
...
func _remove_medal():
	...
	var slot = team_slot_medal_btns.find(currently_selected_medal_btn)
	team_slot_medal_btns[slot] = null
