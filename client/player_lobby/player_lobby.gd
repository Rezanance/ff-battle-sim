...
func _on_team_changed(new_team: int):
	team_index = new_team
	emit_connect_status()

func _on_icon_changed(new_icon_id: int):
	icon_id = new_icon_id

func _on_display_name_changed(new_name: String):
	display_name = new_name.strip_edges()
	emit_connect_status()
	
func _on_server_ip_changed(new_ip: String):
...
func emit_connect_status():
	can_connect.emit(can_go_online())

func can_go_online():
	var is_team_selected = team_index != 0
	var display_name_not_empty = len(display_name) > 0
	var server_ip_correct_format = len(server_ip.split('.')) == 4
