...
var VivosaurSprite = preload("res://client/battle/VivosaurSprite.tscn")
var SkillScene = preload("res://client/team_editor/vivosaur_summary/skill.tscn")
...
func _ready() -> void:
	player_id = Networking.player_info.player_id
	opponent_id = Networking.opponent_info.player_id
...
func create_battlefield():
	...
	var _opponent_slot1 = Networking.opponent_team.slots[0]
	var _opponent_slot2 = Networking.opponent_team.slots[1]
	var _opponent_slot3 = Networking.opponent_team.slots[2]
	...
	battlefield = BattleField.new(formations, true)

	battlefield.support_effects_applied.connect(display_support_effects)

func add_player_vivosaurs():
	var formations = battlefield.formations[Networking.player_info.player_id]
	var vivosaur_az = formations.az
	var vivosaur_sz1 = formations.sz1
	var vivosaur_sz2 = formations.sz2
	...
	player_vivosaur1_sprite_btn.texture_normal = load('res://client/assets/vivosaurs/%d/sprite/%d.png' % [vivosaur_az.vivosaur_info.id, vivosaur_az.vivosaur_info.id])
	player_vivosaur1_sprite_btn.get_node('LifeBar/Bg').texture = load('res://client/assets/lifebars/%d.png' % vivosaur_az.vivosaur_info.element)
	...
	formations.az_sprite_btn = player_vivosaur1_sprite_btn
	if vivosaur_sz1 != null:
		player_vivosaur2_sprite_btn.texture_normal = load('res://client/assets/vivosaurs/%d/sprite/%d.png' % [vivosaur_sz1.vivosaur_info.id, vivosaur_sz1.vivosaur_info.id])
		player_vivosaur2_sprite_btn.get_node('LifeBar/Bg').texture = load('res://client/assets/lifebars/%d.png' % vivosaur_sz1.vivosaur_info.element)
		...
		formations.sz1_sprite_btn = player_vivosaur2_sprite_btn
	else:
		player_vivosaur2_sprite_btn.queue_free()
	if vivosaur_sz2 != null:
		player_vivosaur3_sprite_btn.texture_normal = load('res://client/assets/vivosaurs/%d/sprite/%d.png' % [vivosaur_sz2.vivosaur_info.id, vivosaur_sz2.vivosaur_info.id])
		player_vivosaur3_sprite_btn.get_node('LifeBar/Bg').texture = load('res://client/assets/lifebars/%d.png' % vivosaur_sz2.vivosaur_info.element)
		...
		formations.sz2_sprite_btn = player_vivosaur3_sprite_btn
	else:
		player_vivosaur3_sprite_btn.queue_free()

func add_opponent_vivosaurs():
	var formations = battlefield.formations[Networking.opponent_info.player_id]
	var vivosaur_az = formations.az
	var vivosaur_sz1 = formations.sz1
	var vivosaur_sz2 = formations.sz2
	...
	opponent_vivosaur1_sprite_btn.texture_normal = load('res://client/assets/vivosaurs/%d/sprite/%d.png' % [vivosaur_az.vivosaur_info.id, vivosaur_az.vivosaur_info.id])
	opponent_vivosaur1_sprite_btn.get_node('LifeBar/Bg').texture = load('res://client/assets/lifebars/%d.png' % vivosaur_az.vivosaur_info.element)
	...
	formations.az_sprite_btn = opponent_vivosaur1_sprite_btn
	if vivosaur_sz1 != null:
		opponent_vivosaur2_sprite_btn.texture_normal = load('res://client/assets/vivosaurs/%d/sprite/%d.png' % [vivosaur_sz1.vivosaur_info.id, vivosaur_sz1.vivosaur_info.id])
		opponent_vivosaur2_sprite_btn.get_node('LifeBar/Bg').texture = load('res://client/assets/lifebars/%d.png' % vivosaur_sz1.vivosaur_info.element)
		...
		formations.sz1_sprite_btn = opponent_vivosaur2_sprite_btn
	else:
		opponent_vivosaur2_sprite_btn.queue_free()
	if vivosaur_sz2 != null:
		opponent_vivosaur3_sprite_btn.texture_normal = load('res://client/assets/vivosaurs/%d/sprite/%d.png' % [vivosaur_sz2.vivosaur_info.id, vivosaur_sz2.vivosaur_info.id])
		opponent_vivosaur3_sprite_btn.get_node('LifeBar/Bg').texture = load('res://client/assets/lifebars/%d.png' % vivosaur_sz2.vivosaur_info.element)
		...
		formations.sz2_sprite_btn = opponent_vivosaur3_sprite_btn
	else:
...
func initialize_turn_start_ui():
	...
	opponent_icon.texture = load(icon_path + '/' + icon_files[Networking.player_info.icon_id])
	opponent_name.text = Networking.opponent_info.display_name
...
func display_support_effects(id: int, index: int):
	var support_sprites = battlefield.formations[id].get_support_zones_sprite_btns()
	
	var tween = create_tween()
	player_atk_modifier.text = "%d" % (battlefield.formations[player_id].az_support_effects.atk * 100) + '%'
	player_def_modifier.text = "%d" % (battlefield.formations[player_id].az_support_effects.def * 100) + '%'
	player_acc_modifier.text = "%d" % (battlefield.formations[player_id].az_support_effects.acc * 100) + '%'
	player_eva_modifier.text = "%d" % (battlefield.formations[player_id].az_support_effects.eva * 100) + '%'

	opponent_atk_modifier.text = "%d" % (battlefield.formations[opponent_id].az_support_effects.atk * 100) + '%'
	opponent_def_modifier.text = "%d" % (battlefield.formations[opponent_id].az_support_effects.def * 100) + '%'
	opponent_acc_modifier.text = "%d" % (battlefield.formations[opponent_id].az_support_effects.acc * 100) + '%'
	opponent_eva_modifier.text = "%d" % (battlefield.formations[opponent_id].az_support_effects.eva * 100) + '%'
...
func animate_who_goes_first():
	var _player_total_lp = battlefield.formations[player_id].get_total_lp()
	var _opponent_total_lp = battlefield.formations[opponent_id].get_total_lp()
...
func animate_turn_start(id: int):
	if id == multiplayer.get_unique_id():
		opponent_fp_bg.texture = load("res://client/assets/fp_counter/opp_not_turn.png")
		player_fp_bg.texture = load("res://client/assets/fp_counter/player_turn.png")
	else:
		player_fp_bg.texture = load("res://client/assets/fp_counter/player_not_turn.png")
		opponent_fp_bg.texture = load("res://client/assets/fp_counter/opp_turn.png")
...
func recharge_fp(id: int):
	...
	var delta_fp: int = battlefield.formations[id].fp - old_fp
...
func _on_vivosaur_selected(vivosaur: VivosaurBattle, vivosaur_sprite_btn: TextureButton, is_player_vivo: bool):
	...
	var vivosaur_zone = battlefield.formations[Networking.player_info.player_id].get_vivosaur_zone(vivosaur)
...
func _on_skill_clicked(event: InputEvent, skill: Skill, vivo_zone: Formation.Zone):
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
		currently_selected_vivosaur_sprite_btn.get_node('Arrow').visible = false

		var player_zones = battlefield.formations[player_id]
		...
		var opponent_zones = battlefield.formations[opponent_id]
